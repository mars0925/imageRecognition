...
def plot_images_labels_prediction(images,labels,prediction,idx,pixel,num=num_class):
    ...
    for i in range(0, num):
        ...
        ax.imshow(images[idx].reshape([pixel, pixel]),cmap='binary')
...
def show_Predicted_Probability(y,prediction,x_img,Predicted_Probability,i,pixel):
    print('原來label:',label_dict[y[i][0]],
          ' 預測predict:',label_dict[prediction[i]])
    plt.figure(figsize=(2,2))
    plt.imshow(np.reshape(x_test[i],(pixel,pixel)))
    plt.show()
    for j in range(2):
        print(label_dict[j]+ ' Probability:%1.9f'%(Predicted_Probability[i][j]))
    print("===============finished===========")

show_Predicted_Probability(y_test_OneHot,prediction,x_test_normalize,Predicted_Probability,0,pixel)
show_Predicted_Probability(y_test_OneHot,prediction,x_test_normalize,Predicted_Probability,3,pixel)
